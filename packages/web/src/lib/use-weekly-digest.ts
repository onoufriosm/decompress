...
export interface DigestVideoThumbnail {
  video_id: string;
...
export function useWeeklyDigest() {
  ...
  const [videos, setVideos] = useState<DigestVideoThumbnail[]>([]);
  ...
  const fetchWeeklyDigest = useCallback(async () => {
    ...
    try {
      ...
      // Fetch videos from that week for thumbnails
      ...
      const thumbnails: DigestVideoThumbnail[] = (videoData || []).map((v) => ({
        video_id: v.id,
